...
class TracedFile:
    ...
    def from_traced_file(
        cls, root_dir: Union[str, Path], json_path: Path, repo: LeanRepo
    ) -> "TracedFile":
    ...
    def _get_repo_and_relative_path(self) -> Tuple[LeanRepo, Path]:
    ...
    def get_direct_dependencies(self, repo: LeanRepo) -> List[Tuple[str, Path]]:
    ...
    def from_xml(
        ...
        Args:
            ...
            repo (LeanRepo): The repo to which the traced file belongs.
        """
        root_dir = Path(root_dir)
        path = Path(path)
...
class TracedRepo:
    ...
    dependencies: Dict[str, LeanRepo]
    """Dictionary mapping the name of each dependency to a :class:`LeanRepo` object.
    """
